...
Storybird는 Application Support의 Storybird 루트에 로컬 프로젝트 인덱스와 프로젝트별 자산을
저장한다. 프로젝트 자산을 먼저 완성한 뒤 프로젝트 상태와 인덱스를 원자적으로 교체한다.
원본 영상은 프로젝트 소유의 읽기 전용 자산으로 유지한다.

Storybird 애플리케이션이 프로젝트와 자산의 유일한 작성자다. 사람 UI와 외부 MCP 클라이언트는
Storybird 명령을 통해서만 프로젝트를 변경한다. 인증된 로컬 MCP 연결은 모든 로컬 프로젝트의
...
Storybird는 로컬 음성 프로필 인덱스와 프로필별 참조 음성·대본을 Storybird 루트 안에
저장한다. 생성 내레이션은 프로젝트 소유 자산이며 프로필 삭제와 독립적으로 유지한다.
...
- 앱 재시작 뒤에도 이전에 얻은 revision이 더 최신 상태를 덮어쓰지 못한다 — 재시작 경계의
  동시 편집 계약이다.
- 인증된 로컬 MCP 연결은 모든 로컬 프로젝트의 메타데이터와 편집 모델을 Storybird 명령으로
...
- 앱을 다시 시작한 뒤 오래된 revision으로 요청해도 재시작 이후 프로젝트 상태를 덮어쓰지
  못한다.
- companion의 프로젝트 파일 직접 변경 없이 Storybird 명령을 통해 모든 로컬 프로젝트를
  조회·편집할 수 있다.
...
- 인증된 MCP 클라이언트는 모든 로컬 프로젝트의 편집 모델을 볼 수 있다.
