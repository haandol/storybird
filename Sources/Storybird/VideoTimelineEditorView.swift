...
struct VideoTimelineEditorView: View {
    ...
    @State private var previewPhase: VideoPreviewPhase = .loading
    ...
    var body: some View {
        ...
        .onChange(of: previewLayout.visibility) { _, visibility in
            if visibility == .hidden { previewPhase = .loading }
        }
    ...
    private var playerStage: some View {
        GeometryReader { proxy in
            ...
            let displayPhase: VideoPreviewPhase = playback.isPreparing ? .loading : previewPhase
            ...
            ZStack(alignment: .topLeading) {
                ...
                ZStack(alignment: .topLeading) {
                    ...
                    VideoPlayerSurface(player: playback.player, isPreparing: playback.isPreparing) { phase in
                        let currentPhase: VideoPreviewPhase = playback.isPreparing ? .loading : phase
                        guard previewPhase != currentPhase else { return }
                        if currentPhase == .ready {
                            withAnimation(.easeOut(duration: 0.15)) { previewPhase = currentPhase }
                        } else {
                            var transaction = Transaction()
                            transaction.disablesAnimations = true
                            withTransaction(transaction) { previewPhase = currentPhase }
                        }
                    }
                        .frame(
                            width: frame.width,
                ...
                if displayPhase != .ready {
                    VideoPreviewPlaceholder(phase: displayPhase)
