...
- 목록을 명시하지 않은 연결과 호환 목록을 선택한 연결은 기존 `72개` 도구의 이름·인자·응답·오류·동작 힌트를 보존한다 — 기존 호출 호환 계약이다.
- 통합 목록을 명시적으로 선택한 연결은 같은 기능을 `54개` 도구로 제공한다. 연결별 목록과 서버 안내는 일치하며 연결 중 바뀌지 않는다 — 선택 범위 계약이다.
...
- 같은 기능 집합의 호환 연결은 `72개`, 통합 연결은 `54개`를 나열하고 반복 조회에서도 같은 목록을 반환한다. 한 목록에서만 제공하는 이름을 다른 연결에 보내면 앱 편집 없이 거부한다.
