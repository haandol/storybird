...
클립 구간을 AAC 오디오 트랙 하나로 함께 합성하고, 원본 음성이 없으면 오디오 트랙을 만들지
...
- 원본에 읽을 수 있는 기본 오디오 트랙이 있으면 결과는 AAC 오디오 트랙 `1개`를 가진다 —
  사용자 설명 음성 보존 계약이다.
- 원본에 읽을 수 있는 기본 오디오 트랙이 없으면 결과의 오디오 트랙 수는 `0개`다 — 직접
  화면 녹화의 무음 계약이다.
...
- 영상 클립과 기본 음성의 시작·종료 시간 차이는 원본 영상 `1프레임` 이내다 — 영상·음성
  동기화 목표다.
- 정지 구간과 전체 화면 타이틀·CTA 구간은 AAC 오디오에 같은 길이의 무음 구간을 둔다 —
  카드·정지 화면 출력 계약이다.
...
- 원본 음성이 없는 프로젝트에 새로운 음성이나 시스템 오디오를 추가하지 않는다.
...
- H.264 비디오와 선택적 AAC 기본 음성 이외의 코덱·다중 오디오·인터랙션은 지원하지 않는다.
