...
- 영상이 있는 프로젝트는 추가 표시 동작 없이 프레임을 확인할 수 있어야 하며, 사용자가
  경계를 드래그하거나 미리보기를 숨겨 영상과 레이어에 배정할 세로 공간을 조절할 수 있어야 한다.
...
재생 가능한 영상이 있는 프로젝트 상세는 합성 영상 미리보기와 타임라인을 함께 표시하는
상태로 시작한다. 최초 표시에 별도 표시 동작을 요구하지 않으며 영상을 자동 재생하지 않는다.
사용자는 미리보기를 숨기고 같은 영역 토글로 다시 표시할 수 있다. 프로젝트를 다시 열거나
다른 프로젝트로 전환하면 미리보기를 표시하는 상태로 시작한다.

실제 영상과 타임라인 사이의 경계는 위아래로 드래그하여 두 영역의 높이를
...
선택한 오디오 레이어는 속성 패널에서 단독으로 미리듣는다. 잘라낸 사용 구간의 처음부터
음량·음소거·페이드를 적용하고, 다른 레이어와 원본 영상의 음성은 제외한다. 프로젝트 오디오
목록의 완성 자산·준비 완료 초안은 해당 원본 음성을 미리듣는다. 두 경로는 현재 편집 화면의
미리듣기 하나를 공유하며, 재생 중 같은 버튼으로 중지하고 끝나면 재생 가능한 상태로 돌아간다.
...
- 재생 가능한 영상이 있는 프로젝트 상세를 열거나 다른 프로젝트로 전환하면 실제 합성
  프레임과 타임라인을 함께 표시한다. 최초 표시에 별도 클릭을 요구하지 않고 자동 재생하지
  않는다. 영상이 없거나 읽을 수 없는 경우에는 기존 추가·오류 안내를 유지한다 — 초기 영상 화면 계약이다.
- 실제 영상은 숨길 수 있다. 영역 토글은 숨김 상태에서도 접근 가능하며
  다시 누르면 실제 미리보기를 표시한다. 표시·숨김은 횟수 제한 없이 반복한다 —
  미리보기 가시성 계약이다.
- 실제 영상과 타임라인 사이의 경계를 위아래로 드래그하면 두 영역에
...
- 높이 조절은 타임라인에서 한 번에 보이는 행 수를 바꾸며 개별 레이어 행을 세로로 늘이지
  않는다 — 레이어 표시 밀도 계약이다.
- 미리보기를 표시하면 영상과 타임라인을 함께 사용한다. 표시·숨김 상태 모두 재생·일시정지·탐색과
...
- 오디오 레이어의 속성 패널은 선택 레이어만 사용 구간의 처음부터 재생하고 중지한다.
  음량·음소거·페이드를 적용하며 원본 영상의 음성과 다른 레이어는 포함하지 않는다 — 단독 청취 계약이다.
- 프로젝트 오디오 목록의 완성 자산·준비 완료 초안은 재생 중인 항목에 중지 동작을 표시하고,
  중지·자연 종료·실패 뒤에는 다시 미리듣기 동작을 표시한다 — 재생 상태 가시성 계약이다.
- 한 편집 화면에서 미리듣기는 동시에 최대 `1개`다. 새 항목을 재생하면 기존 미리듣기를
  중지하고 영상 재생을 일시정지한다. 레이어 선택 변경·해당 패널 닫기·프로젝트 전환은
  해당 미리듣기를 종료한다 — 청취 대상과 화면 수명 계약이다.
- 단독 미리듣기와 목록 재생은 원본 자산·프로젝트·수정 버전·실행 취소 기록을 변경하지 않는다.
  읽기·합성·재생 실패는 오류를 표시하고 재시도 가능한 상태로 돌아간다 — 비파괴 청취 계약이다.
- MCP는 유효한 프로젝트 구간의 오디오 미리듣기에 선택 레이어를 지정할 수 있다. 지정하면
  같은 트림·음량·음소거·페이드 규칙으로 해당 레이어만 담은 로컬 파일과 실제 길이·최대 진폭·파형을
  반환한다. 지정하지 않은 기존 요청은 전체 믹스를 유지하며, 없는 레이어는 실패한다 —
  단독 청취 동등성과 기존 호출 보존 계약이다.
...
- 영상이 있는 프로젝트 상세를 처음 열면 별도 표시 클릭 없이 실제 합성 프레임과 타임라인이
  함께 보이며 영상은 일시정지 상태다. 다른 프로젝트를 열어도 같은 초기 표시 규칙을 따른다.
- 실제 영상을 숨길 수 있으며, 숨긴 상태에서도 토글 버튼으로 미리보기를 다시 표시할 수 있다.
- 실제 영상 표시 상태에서 경계를 위아래로 드래그하면 두 영역의 높이가
...
- 겹치는 두 오디오 중 하나를 선택해 미리듣으면 선택한 사용 구간만 해당 음량·페이드로 들린다.
  음소거된 레이어는 소리를 내지 않으며 저장된 음소거 설정은 바뀌지 않는다.
- 목록에서 미리듣기를 누르면 같은 항목에 중지 동작이 나타난다. 끝까지 재생하거나 중지하면
  미리듣기로 돌아가고, 다른 항목을 시작하면 이전 항목은 재생을 멈춘다.
- 레이어 단독 미리듣기 중 선택·패널·프로젝트를 바꾸면 이전 음성은 더 재생되지 않는다.
  실패 뒤에는 오류를 확인하고 같은 항목을 다시 재생할 수 있다.
- UI 단독 미리듣기와 같은 프로젝트 구간·레이어를 지정한 MCP 파일은 같은 음성을 담는다.
  전체 믹스 요청과 단독 요청 후에도 프로젝트·수정 버전·실행 취소 기록은 같다.
...
미리보기 표시 상태는 프로젝트의 편집 상태와 독립적이다. 실제 영상 표시 상태에서 경계를
드래그하면 표시 상태는 유지하고 두 영역의 높이만 바뀐다.

```mermaid
stateDiagram-v2
    state "미리보기 숨김 · 타임라인 확장" as Hidden
    state "미리보기 표시 · 영상과 타임라인 함께 사용" as Visible
    [*] --> Visible: 영상이 있는 프로젝트 상세 열기
...
- 프로젝트를 열면 바로 영상 프레임을 확인하고, 경계 드래그나 미리보기 숨김으로 더 많은
  레이어를 한 번에 볼 수 있다.
...
- 프로젝트 진입부터 실제 영상 표면을 표시하므로 초기 미리보기 준비 비용이 발생한다.
