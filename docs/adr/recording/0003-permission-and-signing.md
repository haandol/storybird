...
Input Monitoring은 왼쪽·오른쪽 mouse-down 관찰에만 사용한다. Storybird는 키보드, 마이크,
LLM, 모델 API, 모델 API 키 또는 자체 외부 네트워크 경로를 사용하지 않는다.
...
- Storybird는 마이크 권한을 요청하거나 음성을 캡처하지 않는다.
...
- Storybird와 companion은 외부 제어용 TCP·HTTP 서버 또는 Storybird 소유 외부 네트워크
  경로를 만들지 않는다.
...
- 키보드·마이크 권한, 모델 API 키와 네트워크 연결 없이 사람 녹화와 로컬 MCP 프로젝트 편집을
  사용할 수 있다.
