...
- 외부 음성 파일과 직접 마이크 녹음을 모두 지원해야 한다.
...
WAV와 정확한 참조 대본이거나, Storybird가 제시한 대본을 읽은 `3초` 이상의 명시적 마이크
녹음이다. Storybird는 참조 음성, 대본과 프로필을 기기 안에 저장한다.
...
- 마이크 음성 프로필은 Storybird가 제시한 대본과 `3초` 이상의 녹음을 요구한다 — 최소 참조
  음성 계약이다.
...
- 제시 대본을 `3초` 이상 읽으면 녹음 미리듣기와 재녹음 후 프로필이 나타난다.
