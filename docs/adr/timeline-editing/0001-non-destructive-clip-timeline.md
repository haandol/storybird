...
프로젝트 소유 내레이션은 프로젝트 시각 고정 또는 시작 장면 연결을 선택하는 시간 레이어다. 내레이션은 문장별
오디오 자산, 원고, 음성 프로필, 시작 시각, 재생 길이와 음량을 가지며 서로 겹칠 수 있으며 프로젝트 종료를 넘을 수 없다. 원고 재생성은 대상 내레이션 자산과 레이어를 원자적으로 교체한다.

### Requirement contract

#### Required guarantees

- 오디오 레이어는 생성 음성, 직접 녹음 또는 가져온 자산의 사용 구간을 독립적으로 배치한다. 레이어 간 겹침과 동시 재생을 허용하며 행 순서는 음성 우선순위를 바꾸지 않는다 — 다중 레이어 계약이다.
- 각 레이어는 고유 식별자, 자산 식별자, 이름, 원본 시작·사용 길이, 프로젝트 시작, 음량, 음소거, 페이드 인·아웃과 선택적 장면 연결을 가진다. 시간 단위는 초다 — 편집 의미 계약이다.
- 이동·트림·분할·복제·삭제·음량·음소거·페이드를 편집한다. 원본 구간은 자산 범위 안의 양수 길이이고 프로젝트 시작은 0초 이상, 끝은 프로젝트 끝 이하다. 분할은 구간 내부에서만 허용한다 — 유효 구간 계약이다.
- 음량은 유한한 0 이상의 배율이며 1이 원래 음량이다. 기본값은 음량 1, 음소거 해제, 페이드 0초다. 페이드는 선형이며 각 길이는 0초 이상, 두 길이의 합은 레이어 길이 이하다 — 음량 의미 계약이다.
- 고정 시각과 시작 장면 연결을 선택한다. 독립 오디오 길이·속도·음높이는 영상 편집으로 바꾸지 않는다. 장면 편집 뒤 중첩은 허용하고 프로젝트 초과는 전체 편집을 거부한다 — 비파괴 장면 동기화 계약이다.
- 영상 기본 오디오는 영상 구간을 따라가며 음량과 음소거를 편집할 수 있다 — 기본 음성 조절 계약이다.


- 내레이션과 독립 자막의 배치 방식은 프로젝트 시각 고정 또는 장면 연결이다. 연결 정보가 없는 기존 데이터와 기존 MCP 요청은 고정 시각으로 해석한다. 새 UI에서 장면 위에 추가하는 레이어는 장면 연결을 기본으로 하며 고정 시각을 선택할 수 있다 — 기존 의미 보존 계약이다.
- 장면 연결은 시작점의 소유 클립과 원본 시각을 보존하고 정지 클립은 클립 안의 초 단위 위치를 보존한다. 카드에는 고정 시각만 배치한다 — 시작점 소유권 계약이다.
- 클립 이동·앞 구간 편집·속도 변경은 연결 레이어 시작점을 같은 장면으로 이동한다. 내레이션 실제 길이·속도·음높이와 자막 표시 길이는 자동 변경하지 않는다 — 발화 보존 계약이다.
- 분할 경계의 시작점은 오른쪽 클립 하나에 귀속한다. 시작점이 트림되거나 소유 클립이 삭제되면 연결 레이어를 제외하고 실행 취소 때 복원한다. 복원에 필요한 음성 자산은 보존한다 — 비파괴 연결 수명 계약이다.
- 발화가 다음 장면까지 이어지는 것은 허용한다. 자동 이동이 프로젝트 종료 초과를 만들면 편집 전체를 거부한다 — 동기화 원자성 계약이다.
- 타이틀 삽입·삭제는 뒤 레이어를 카드 길이만큼 이동하며 UI와 MCP는 같은 연결·검증·저장·실행 취소 결과를 사용한다 — 경로 동등성 계약이다.
...
- UI와 MCP가 같은 편집 명령을 적용하면 같은 클립 순서, 프로젝트 시간과 연결 레이어 시간을
  만든다 — 편집 경로 동등성 계약이다.
- 내레이션 레이어는 중첩을 허용하며 프로젝트 시간 범위 안에 있다 — 설명 음성 가독성 계약이다.
