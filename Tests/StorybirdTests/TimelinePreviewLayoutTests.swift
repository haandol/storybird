...
final class TimelinePreviewLayoutTests: XCTestCase {
    func test_audioInspectorAudition_wholeWorkspaceRemainsResponsive() async throws {
        let (store, initial) = try await fixture()
        var project = initial
        project.subtitles = []
        project = try store.saveProject(project, expectedRevision: initial.revision)
        let sound = store.repository.rootURL.appendingPathComponent("audition.wav")
        try TestVideoFactory.makeToneWAV(at: sound, duration: 2)
        ...
        let window = NSWindow(contentRect: NSRect(x: -10_000, y: -10_000, width: 1100, height: 760),
        ...
        let point = NSPoint(x: frame.minX + bridge.bounds.width / CGFloat(project.timelineDuration),
                           y: frame.maxY - (52 + CGFloat(row) * 44 + 19))
        sendMouse(.leftMouseDown, at: point, in: window)
        sendMouse(.leftMouseUp, at: point, in: window)
        await settle()
        try snapshot(view, name: "audio-audition-before")
        let surface = try XCTUnwrap(descendants(view).compactMap { $0 as? AVPlayerView }.first)
        for startVideo in [false, true] {
            if startVideo {
                let controls = try playbackFrame(view)
                let play = NSPoint(x: controls.minX - 30, y: controls.midY)
                sendMouse(.leftMouseDown, at: play, in: window)
                sendMouse(.leftMouseUp, at: play, in: window)
                await settle()
                XCTAssertGreaterThan(surface.player?.rate ?? 0, 0)
            }
            let listen = NSPoint(x: 905, y: 570)
            sendMouse(.leftMouseDown, at: listen, in: window)
            sendMouse(.leftMouseUp, at: listen, in: window)
            await settle()
            XCTAssertEqual(surface.player?.rate, 0, "Layer audition must pause the video.")
            try snapshot(view, name: "audio-audition-playing-\(startVideo)")
            try await Task.sleep(for: .seconds(3))
            try snapshot(view, name: "audio-audition-finished-\(startVideo)")
    ...
    func test_previewVisibility_startsVisibleAndRepeatedTogglesRestoreActualPreview() {
    ...
    func test_hideDuringResize_preservesPreferredHeightUntilShownAgain() {
        ...
        XCTAssertEqual(layout.visibility, .visible)
        XCTAssertEqual(layout.previewHeight(totalHeight: 760, minimumTimelineHeight: 208), 330)
        XCTAssertEqual(TimelinePreviewLayout().visibility, .visible, "A new editor does not inherit this view choice.")
    }
    ...
    func test_previewUI_autoDisplayToggleAndResizePreserveProjectInWideAndCompactWindows() async throws {
        ...
        for width in [1100.0, 760.0] {
            ...
            try snapshot(view, name: "initial-visible-\(Int(width))")

            // The automatically displayed video can still yield space.
    ...
    func test_previewOpen_loadsActualPlayerWithoutClickOrAutoplay() async throws {
        let (store, project) = try await fixture()
        ...
        defer { window.close() }
        await settle()
        try snapshot(view, name: "initial-preview")
        let surface = try XCTUnwrap(descendants(view).compactMap { $0 as? AVPlayerView }.first)
        XCTAssertNotNil(surface.player?.currentItem)
        for _ in 0..<100 where !surface.isReadyForDisplay {
            try await Task.sleep(for: .milliseconds(20))
        }
        XCTAssertTrue(surface.isReadyForDisplay, "The first video frame must be ready without a Show or Play click.")
        XCTAssertEqual(surface.player?.rate, 0)
        XCTAssertEqual(store.project(id: project.id), project)
    }

    func test_unreadableVideo_showsUnavailableWorkspace() async throws {
        let (store, project) = try await fixture()
