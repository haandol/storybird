...
`--check`는 push 없는 검사다. 일반 `git push` 자체에 공개 hook을 설치하지 않는다.
사용자가 준비된 버전의 push 완료를 알리면 에이전트가 로컬 macOS의 기존 `gh` 인증으로
공개를 이어서 실행한다. 컨테이너의 API 인증 설정을 요구하거나 호스트 토큰을 복사하지 않는다.
동일한 공개 릴리즈는 내용·자산을 검증하고 변경 없이 끝낸다. 재시도로 다른 버전의 Latest를
되돌리지 않는다.
...
준비된 버전 태그의 push 완료 알림은 공개 진행 신호이므로 공개 여부를 다시 묻지 않는다.
