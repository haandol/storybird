...
저장한다. 기본 위치는 `~/Documents/Storybird`다. Finder에서 프로젝트를 찾고 백업하기
쉬운 문서 폴더를 사용한다. 프로젝트 자산을 먼저
...
Application Support에 Storybird 루트가 없고 OpenLane 루트가 있으면 그 위치에 복사해 원본을 보존한다.
Application Support의 프로젝트는 문서 폴더로 이동하거나 병합하지 않으며 설정에서 해당 폴더를
선택해 접근한다.
새 편집 모델은 기존 프로젝트를 읽거나 변환하거나 내보낼 의무가 없다. 기존 파일은 삭제하거나
이동하지 않고 그대로 둔다.

Storybird는 공유 음성 프로필 인덱스와 프로필별 참조 음성·대본, 준비한 음성 모델을
Application Support의 Storybird 루트 안에 유지한다. 프로젝트 폴더 전환은 이 공유 자산이나 로컬 MCP 연결 위치를
...
- 미디어 가져오기는 필수 요청 식별자와 입력 종류·절대 경로·대상 프로젝트를 선택한 라이브러리에 보존한다. 같은 키와 같은 입력은 기존 작업과 결과를 반환하고 다른 입력은 충돌로 거부한다 — 중복 없는 재시도 계약이다.
- 가져오기 작업 상태는 가져오는 중, 완료, 실패, 취소다. 가져오는 중에서만 종료 상태로 전환하며 종료 상태는 재실행하지 않는다. 실패·취소 후 새 시도는 새 요청 식별자를 사용한다 — 작업 수명 계약이다.
- 작업과 요청 식별자는 라이브러리에 보존하며 앱 재시작·폴더 재선택 뒤에도 조회한다. 중단된 작업은 자동 재실행하지 않고, 이미 공개된 결과가 있으면 완료로 복구하고 그렇지 않으면 실패와 부분 자산 정리로 복구한다 — 재시작 중복 방지 계약이다.
- 가져오기 시작은 작업 식별자를 반환하고 조회·취소를 제공한다. 완료일 때만 결과 프로젝트 또는 자산 식별자와 실제 길이를 반환한다. 원본은 변경하지 않고, 결과가 삭제되어도 같은 요청을 새 생성으로 해석하지 않는다 — 결과 식별 계약이다.
- 가져오기 작업은 편집 revision·실행 취소와 분리한다. 복사·미디어 검증·저장 실패나 취소에서 기존 프로젝트를 유지하고 미완성 자산을 공개하지 않는다 — 저장 실패 보존 계약이다.

- 프로젝트는 검증된 녹음·가져온 오디오와 생성 음성을 소유하며 자산 식별자, 이름, 실제 길이와 출처를 보존한다. 가져오기 입력은 WAV·MP3·M4A다 — 독립 음성 준비 계약이다.
- 완성 자산 등록은 편집 revision과 재생에 영향을 주지 않으며 여러 레이어에서 재사용할 수 있다. 배치 실패 후에도 자산을 유지한다 — 준비와 배치 분리 계약이다.
- 레이어 트림·분할·삭제는 오디오 원본을 변경하지 않는다. 프로젝트의 자산과 실행 취소가 참조하는 오디오는 유지한다 — 비파괴 자산 수명 계약이다.
- 프로젝트 복제는 배치된 오디오의 전체 원본을 새 프로젝트 소유로 복사하고 미배치 자산·초안과 실행 취소 기록은 복사하지 않는다 — 독립 복제 계약이다.
- 현재 지원하는 프로젝트의 기존 내레이션은 시간·음량·언어·프로필·오디오를 보존해 읽는다. 새 오디오 편집 속성의 기본값은 원본 시작 0초, 음소거 해제, 페이드 0초다 — 현재 형식 호환 계약이다.


- UI와 인증된 MCP는 현재 영상 프로젝트를 같은 라이브러리의 독립 프로젝트로 복제할 수 있다. 원본 revision을 확인하고 원본 영상과 배치된 내레이션을 새 프로젝트 소유로 복사한 뒤 검증·저장을 모두 마쳤을 때만 공개한다. 새 revision은 `0`이다 — 언어별 독립 편집 계약이다.
- 복제본은 편집 상태와 언어·음성 프로필 선택을 유지한다. 미배치 음성 초안, 실행 취소 기록과 진행 중 작업은 복제하지 않는다. 원본 수정·삭제·재생성은 복제본에 영향을 주지 않는다 — 자산 수명 분리 계약이다.
- 복제 실패와 오래된 revision은 원본과 기존 라이브러리를 유지하고 미완성 복제 자산을 정리한다 — 실패 원자성 계약이다.
- 합성 음성 초안은 대상 프로젝트와 같은 라이브러리에 보관한다. 준비 완료 초안은 앱 재시작과 폴더 재선택 뒤에도 남으며 다른 라이브러리로 이동·병합하지 않는다. 프로젝트 삭제는 소속 초안도 제거한다 — 초안 소유권 계약이다.
- 미배치 초안은 프로젝트의 재생·출력과 편집 revision을 변경하지 않는다. 배치는 최신 revision에서 한 레이어·한 revision·한 실행 취소 단위로 저장하고 중복 배치를 거부한다 — 초안과 편집 상태 분리 계약이다.
- 음성 생성·배치 중에는 저장 폴더 전환을 막는다. 저장 또는 배치 실패는 마지막 유효 프로젝트와 준비 완료 초안을 유지한다 — 작업 위치·실패 보존 계약이다.

- 기본 프로젝트 루트와 기본 위치 복원의 대상은 `~/Documents/Storybird`다 — Finder에서
  찾기 쉬운 저장 위치 계약이다.
- 저장된 사용자 지정 폴더는 기본 위치 변경과 무관하게 유지한다. Application Support의 기존
  프로젝트는 자동 이동·삭제·병합하지 않고 해당 폴더를 선택해 접근한다 — 기존 데이터 보존 계약이다.
...
- 프로젝트 인덱스, 원본 영상, 생성 내레이션과 편집 상태는 선택한 프로젝트 폴더에 저장한다.
  공유 음성 프로필·참조 음성·대본·모델과 로컬 MCP 연결 위치는
  `~/Library/Application Support/Storybird`에 유지한다 —
...
- Application Support에 Storybird 루트가 없고 OpenLane 루트가 있으면 Storybird 루트로 복사하고 원본을
  남긴다 — 기존 데이터 보존 계약이다.
- Application Support에 Storybird 루트가 이미 있으면 OpenLane 데이터로 덮어쓰지 않는다 — 현재 라이브러리
  우선 계약이다.
...
- 같은 가져오기를 동시 요청하거나 응답 유실 뒤 재전송해도 프로젝트 또는 오디오 자산은 하나다. 같은 키의 다른 입력은 거부된다.
- 결과 저장 직후 앱이 종료되어도 재시작 후 같은 요청은 이미 저장한 결과를 찾는다. 저장 전 중단은 실패로 보이며 재촬영이나 가져오기를 자동으로 시작하지 않는다.

- 분리한 음성 자산을 겹쳐 배치하고 저장·재실행하면 각 레이어의 사용 구간과 음량이 유지된다.
...
- companion의 프로젝트 파일 직접 변경 없이 Storybird 명령을 통해 현재 선택한 라이브러리의 모든 프로젝트를
  조회·편집할 수 있다.
- Application Support에 Storybird 루트가 없는 첫 실행에서 OpenLane 파일은 같은 Application Support의 Storybird 쪽에 복사되며 원본 루트는
  그대로 남는다.
...
- 사용자 지정 폴더가 없는 실행과 기본 위치 복원은 문서 폴더의 Storybird 라이브러리를 표시한다.
...
#### 프로젝트와 공유 자산을 모두 Application Support에 저장한다

앱 내부 자산을 한곳에 모을 수 있지만 사용자가 Finder에서 프로젝트를 찾고 백업하기 어렵다.
프로젝트는 문서 폴더에 두고 공유 음성 환경과 로컬 연결 위치는 Application Support에 유지한다.
