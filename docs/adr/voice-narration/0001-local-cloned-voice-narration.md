...
Storybird 설정의 음성 영역은 로컬 모델 준비, 프로필 등록·미리듣기·삭제와 안내 녹음용 마이크
선택을 한곳에서 관리한다. 마이크 선택의 기본값은 macOS 시스템 기본 입력을 따르는 상태다.
...
- 안내 녹음은 선택한 참조 언어의 `10초...15초` 대본을 표시하고 녹음 중 언어·대본을 바꾸지 않는다. 기존 `10초` 최소 녹음과 사용 동의를 유지한다 — 참조 일관성 계약이다.
...
- 길이 초과·중첩·revision 충돌·저장 실패는 기존 프로젝트와 준비 완료 초안을 유지한다. 영상 길이를 자동으로 늘리거나 다른 발화를 밀지 않고 사용자 또는 에이전트가 장면을 편집한 뒤 재합성 없이 다시 배치한다 — 배치 실패 보존 계약이다.
...
- 로컬 모델 준비, 음성 프로필 등록·미리듣기·삭제와 안내 녹음용 마이크 선택은 Storybird
...
- 타임라인에는 겹치는 내레이션, 프로젝트 범위 밖 내레이션과 실패한 생성 자산을 저장하지 않는다. 미배치 준비 완료 초안은 타임라인과 분리해 보관한다.
