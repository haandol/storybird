# ADR 0001: 로컬 모델로 목소리를 복제하거나 내장 화자로 내레이션을 생성한다
...
음성 프로필을 만들지 않고 내장 화자로 바로 생성하는 경로도 필요하다. 사용자는 화자를
선택하고 지시어로 말투를 조절하며, 나중에 원고를 수정할 때도 같은 생성 조건을 유지해야 한다.
...
- Apple Silicon Mac 안에서 본인 목소리 복제와 프로필 없는 내장 화자 생성을 모두 제공해야 한다.
- 문장 하나의 수정은 저장된 화자·지시어로 해당 문장만 재생성하고 다른 내레이션을 보존해야 한다.
- 에이전트는 기존 프로필 또는 내장 화자를 사용하며, 참조 파일·마이크 입력과 프로필 관리 권한은 사용자에게 남겨야 한다.
- 모델 설치·제거와 합성의 실패가 프로젝트, 다른 모델이나 기존 음성 자산을 부분 변경하면 안 된다.
- 사용자가 외부 파일 또는 직접 녹음으로 프로필을 만들고, 선택한 마이크의 지속성과 장치 부재 시 녹음 진입을 유지해야 한다.

## Decision

Storybird는 통합 메모리 `24GB` 이상 Apple Silicon Mac을 PoC 기준 환경으로 사용하고,
`Qwen3-TTS 12Hz 1.7B Base 8-bit`와 `Qwen3-TTS 12Hz 1.7B CustomVoice 8-bit` MLX 모델을
로컬 음성 생성 경계로 사용한다. Base는 사용자의 프로필로 목소리를 복제하고 CustomVoice는
내장 화자와 선택적 지시어로 프로필 없이 생성한다. 사용자는 Settings 또는 인증된 MCP에서 모델 상태를 조회하고
선택하며 준비를 요청한다. 기본 선택은 1.7B Base이고 앱 재실행 후에도 선택을 유지한다.
...
설정은 모델 설치와 제거 버튼을 제공한다. 인증된 MCP도 같은 앱 명령으로 특정 모델을
제거하며 추가 승인 창을 요구하지 않는다. 제거 대상은 그 모델의 앱 소유 실행 환경과 다운로드
파일이다. 다른 모델, 음성 프로필, 완성 오디오와 프로젝트는 보존한다. 선택된 모델을 제거해도
선택은 유지하고 재설치 전 해당 모델을 사용하는 새 음성 생성을 거부한다. 모델 설치·합성·제거와 모델 변경이
서로 충돌하지 않도록 작업 중 변경을 거부하며, 제거 결과와 실패를 두 경로에서 조회한다.
...
합성 결과는 프로젝트 소유의 문장별 내레이션 오디오 레이어다. 각 레이어는 원고, 복제 프로필 또는 내장 화자·지시어,
언어, 프로젝트 시작 시각, 재생 길이와 음량을 가지며 다른 내레이션과 겹칠 수 있으며 프로젝트 종료를 넘을 수 없다. 원고를 바꾸면 해당 문장만 새 자산으로 생성하고 성공한 자산을 저장한 뒤
프로젝트를 원자적으로 교체한다. 프로젝트 내 내레이션 작성 화면은 복제 프로필 또는 내장 화자를
선택하며 모델 준비, 프로필 등록·삭제와 마이크 장치 변경을 수행하지 않는다.

인증된 로컬 MCP 연결은 프로필 목록 조회와 복제 프로필 또는 내장 화자·지시어를 사용한 내레이션 생성·수정·삭제,
프리뷰와 내보내기를 수행할 수 있다. 모델 상태 조회·선택·설치·제거도 UI와 동일한 앱 명령으로 수행한다.
...
- 새 음성 생성 모델은 1.7B Base 8비트와 1.7B CustomVoice 8비트다. 0.6B는 새 선택·설치·생성에서 제외하고 남은 로컬 모델 파일은 설정과 인증된 MCP에서 제거할 수 있다. 저장된 0.6B 선택은 1.7B Base로 전환하고 파일을 자동 삭제하지 않는다 — 지원 모델과 기존 설치 정리 계약이다.
- UI는 내 목소리 복제와 기본 목소리 생성을 선택하게 한다. 복제는 Base와 사용 동의가 있는 기존 프로필을 요구하고, 기본 목소리는 CustomVoice 내장 화자를 요구하며 프로필·참조 음성·녹음을 요구하지 않는다 — 생성 입력 구분 계약이다.
- 내장 화자는 Vivian, Serena, Uncle_Fu, Dylan, Eric, Ryan, Aiden, Ono_Anna, Sohee다. UI와 MCP에서 화자를 조회·선택하고 한국어·영어 원고와 선택적 지시어로 음성을 생성한다. 빈 지시어는 별도 스타일 지시 없이 생성한다 — 화자와 스타일 제어 계약이다.
- MCP는 기본 목소리 생성 시 화자와 지시어를 받고, 이미 생성한 기본 목소리 레이어의 지시어·화자를 변경하여 다시 합성할 수 있다. 지시어만 바꾼 재생성은 저장된 원고·언어를 유지한다. 복제 요청에 내장 화자·지시어를 함께 전달하거나 유효한 음성 입력이 없으면 저장 없이 거부한다 — 자동화 입력 계약이다.
- 초안·오디오 자산·레이어는 복제 프로필 또는 기본 목소리의 화자·지시어를 원고·언어와 함께 보존한다. 배치·복제·분할·재사용·프로젝트 복사와 재실행 뒤에도 같은 생성 방식으로 원고를 재생성할 수 있다. 기존 데이터는 복제 프로필과 완성 오디오의 의미를 유지한다 — 생성 조건 지속성 계약이다.
- 화자·지시어 재생성은 원고 재생성과 같은 revision 검증·원자 저장·실행 취소 규칙을 사용한다. 오래된 revision, 합성 실패와 저장 실패는 이전 오디오와 생성 조건을 보존한다 — 재생성 실패 보존 계약이다.
...
- 생성 음성은 재생성용 원고·언어와 복제 프로필 또는 내장 화자·지시어를 유지한다. 직접 녹음·가져온 오디오는 원고와 프로필을 요구하지 않는다 — 입력 종류 구분 계약이다.
...
- 음성 복제 모델은 `Qwen3-TTS 12Hz 1.7B Base 8-bit`와 `Qwen3-TTS 12Hz 1.7B CustomVoice 8-bit`다. 두 모델 모두 8비트이며 기본 선택은 1.7B Base다 — 승인된 모델 집합이다.
...
- 모델 상태는 모델별 미준비·준비 중·준비 완료·제거 중·실패다. 비동기 설치·제거의 완료·실패와 오류 사유를 UI와 MCP에서 조회하고 실패 후 재시도할 수 있다. 같은 모델의 진행 중 중복 요청은 추가 작업을 만들지 않는다 — 모델 수명주기 계약이다.
- 설정은 선택한 모델의 설치와 제거 버튼을 모두 제공한다. 인증된 MCP도 모델 식별자로 같은 설치·제거 명령을 실행하며 명시적 요청 뒤 추가 승인 창을 요구하지 않는다 — 모델 관리 동등성 계약이다.
- 제거는 해당 모델의 앱 소유 실행 환경과 다운로드 파일만 대상으로 한다. 다른 모델, 음성 프로필, 참조 음성, 완성 오디오, 준비 완료 초안과 프로젝트를 보존한다 — 제거 범위 계약이다.
- 제거 성공은 미준비 상태가 되고 이미 제거된 모델의 반복 제거도 성공한다. 선택된 모델의 제거는 지속 선택을 유지하며 재설치 전 해당 모델을 사용하는 새 음성 생성을 거부하고 자동으로 다른 모델을 사용하지 않는다 — 제거 후 동작 계약이다.
- 설치·합성·제거 중 다른 모델 선택·설치·제거를 거부하며, 설치·제거 중 새 합성이 모델 파일을 사용하지 않는다. 진행 중 모델 관리 작업은 저장 폴더 변경도 거부한다 — 작업 충돌 방지 계약이다.
- 모델 제거는 프로젝트 revision과 실행 취소 기록을 바꾸지 않는다 — 프로젝트 편집 분리 계약이다.
- 음성 작업 중 모델 선택 변경을 거부한다. 생성 방식에 따라 Base 또는 CustomVoice를 사용하며 원고 재생성은 저장된 생성 방식을 사용한다. 준비되지 않은 모델을 다른 모델로 자동 대체하지 않는다 — 생성 모델 일관성 계약이다.
- 두 모델을 동시에 보관할 수 있고 명시적으로 제거한 모델 외의 준비 자산·음성 프로필·완성 오디오를 보존한다. 모델 선택과 준비는 프로젝트 revision과 실행 취소를 바꾸지 않는다 — 공유 자산 보존 계약이다.
...
- 생성 모달의 긴 내용은 내부에서 스크롤하며 완료·취소 작업에 도달할 수 있다 —
  생성 작업 도달성 계약이다.
- 프로젝트 내 내레이션 작성은 설정에서 만든 기존 프로필 또는 CustomVoice 내장 화자를 사용한다 — 영구 음성 설정과
  프로젝트 편집의 분리 계약이다.
...
- 모델 제거 실패는 성공으로 표시하지 않고 오류를 제공하며 재시도할 수 있다. 부분 제거된 실행 환경은 준비 완료로 사용하지 않는다. 재시작 뒤 로컬 결과를 다시 확인하고 설치나 합성을 자동 실행하지 않는다.
...
- 프로필이 없는 상태에서 기본 목소리를 선택하고 화자·원고·언어·지시어를 입력하여 UI와 MCP로 음성 초안을 생성한다. 복제 모드는 기존 프로필을 요구한다.
- 같은 기본 목소리 레이어의 지시어만 바꿔 재생성하면 원고·언어를 유지하고 새 오디오와 지시어가 함께 저장된다. 실행 취소는 이전 오디오와 지시어를 복원한다.
- 기본 목소리 오디오를 배치·복제·분할·재사용하고 프로젝트를 복사·재실행한 뒤에도 화자·지시어가 남는다. 잘못된 화자나 복제 입력과의 혼합은 어떤 오디오나 프로젝트도 변경하지 않는다.
- 기존 0.6B 선택은 Base로 바뀌고 신규 설치·선택 요청은 거부된다. 남아 있는 0.6B 파일은 제거할 수 있으며 기존 오디오와 프로필은 유지된다.

- 설정의 설치·제거 버튼과 MCP로 같은 모델을 설치·제거하고 진행 상태와 완료·실패를 조회한다. 제거 뒤 미준비 상태와 기존 선택을 확인하고, 반복 제거 뒤에도 성공하며 재설치할 수 있다.
- 선택 모델 제거 뒤 기존 음성과 준비 완료 초안은 재생·배치할 수 있지만 해당 모델의 새 합성은 재설치 전까지 실패한다. 다른 모델·프로필·프로젝트 revision·실행 취소 기록은 동일하다.
- 설치·합성·제거 중 모델 변경과 저장 폴더 변경은 거부된다. 제거 실패는 오류와 재시도를 제공하고 부분 제거한 모델을 준비 완료로 사용하지 않는다.
- Settings와 MCP에서 Base·CustomVoice 8비트 모델의 상태를 확인하고 선택을 바꾼 뒤 앱을 다시 열어도 같은 선택을 볼 수 있다.
- UI의 준비 버튼과 MCP 준비 요청 뒤 승인 창 없이 준비 중 상태를 받고 완료 또는 실패를 조회한다. 같은 준비 요청을 반복해도 다운로드가 중복되지 않는다.
- 두 모델을 준비한 뒤 네트워크 없이 Base는 기존 프로필로, CustomVoice는 내장 화자로 합성한다. 한 모델의 준비 실패와 선택 변경이 다른 모델·기존 프로필·완성 오디오·프로젝트 revision을 바꾸지 않는다.
...
sequenceDiagram
    ...
    User->>Storybird: 복제 프로필 또는 내장 화자·지시어와 원고 선택
    Agent->>Storybird: 같은 생성 조건으로 내레이션 요청
    Storybird->>Voice: Base 복제 또는 CustomVoice 로컬 합성
